...
import com.zyb.apiCommon.model.vo.UserVO;
...
import com.zyb.backend.exception.ThrowUtils;
...
public class UserController {
    ...
    public BaseResponse<Boolean> regenerateKey(HttpServletRequest request) {
        if (request == null) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
    ...
    @AuthCheck(mustRole = UserConstant.ADMIN_ROLE)
    public BaseResponse<Boolean> addUserBalance(@RequestBody UserUpdateMyRequest userUpdateMyRequest) {
        if (userUpdateMyRequest == null || userUpdateMyRequest.getId() <= 0) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }

        BigDecimal amountToAdd = userUpdateMyRequest.getBalance();
        if (amountToAdd == null || amountToAdd.compareTo(BigDecimal.ZERO) <= 0) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "充值金额必须为正数");
        }

        User user = userService.getById(userUpdateMyRequest.getId());
        if (user == null) {
